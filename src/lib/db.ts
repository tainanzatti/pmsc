...
export function movingAverageMastery(lancamentos: Lancamento[], topicId: string, windowSize = 5): number {
  ...
  entries.forEach((entry, idx) => {
    const weight = entries.length - idx;
...
export async function getFlashcardQueueTopics(): Promise<{ topic: TopicRow; mastery: number }[]> {
  ...
  return belowThreshold
    .map((t) => { const row = topicRows.find((r) => r.id === t.id); return row ? { topic: row, mastery: t.mastery } : null; })
...
export async function getNewTopicsPerDayLast30Days(): Promise<number> {
  ...
  for (const r of rows) {
    const existing = firstTouchByTopic.get(r.topic_id);
    if (!existing || r.created_at < existing) firstTouchByTopic.set(r.topic_id, r.created_at);
  }
  const uniqueDays = new Set<string>();
  for (const dateStr of firstTouchByTopic.values()) {
...
export async function getAvgMinutesPerNewTopic(): Promise<number> {
  ...
  const [lancRes, sessionRes] = await Promise.all([
    supabase.from("lancamentos").select("topic_id, created_at").gte("created_at", sinceStr),
  ...
  if (newTopicCount === 0) return 30; // default 30 min per topic
