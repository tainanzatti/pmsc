...
import {
  DISCIPLINES, type DisciplineId,
  type DisciplineSkipState, type TopicWithMastery,
  consolidateTopics,
  masteryToTier, nextHeroDiscipline, scoreDisciplines,
} from "../../lib/curriculum";
...
export default function PainelView({ onNavigate }: PainelViewProps) {
    ...
    try {
      ...
      for (const t of topicRows) {
        if (!topicMap.has(t.id)) {
          topicMap.set(t.id, { id: t.id, discipline_id: t.discipline_id as DisciplineId, title: t.title, mastery: 0, last_reviewed_at: null });
        } else {
      ...
      const [mToday, mTotal, fcCount, untouched, newPerDay, avgMin] = await Promise.all([
        ...
        countDueFlashcards(),
        getUntouchedTopicCount(),
        getNewTopicsPerDayLast30Days(),
        getAvgMinutesPerNewTopic(),
      ]);
      setMinutesToday(mToday);
      setTotalMinutes(mTotal);
      setFlashcardCount(fcCount);
      setUntouchedCount(untouched.untouched);
      setTotalTopics(untouched.total);
      setNewTopicsPerDay(newPerDay);
      setAvgMinutesPerTopic(avgMin);

      // Due reviews: topics with last_reviewed_at older than 7 days
      const allTopics = Array.from(topicMap.values());
      const now = new Date();
      const overdue = allTopics
        .filter((t) => t.last_reviewed_at)
        .map((t) => ({
          topicId: t.id,
          disciplineId: t.discipline_id,
          days: Math.floor((now.getTime() - new Date(t.last_reviewed_at as string).getTime()) / (1000 * 60 * 60 * 24)),
        }))
        .filter((r) => r.days >= 7)
  ...
  const goodPct = useMemo(() => {
    if (topics.length === 0) return 0;
    const good = topics.filter((t) => {
      const tier = masteryToTier(t.mastery);
  ...
  // Projection: untouched topics / new topics per day
  const projectionDays = useMemo(() => {
    if (newTopicsPerDay <= 0) return Infinity;
    return Math.ceil(untouchedCount / newTopicsPerDay);
  }, [untouchedCount, newTopicsPerDay]);

  // Calculator: slider hours/day → days to cover all untouched
  const calculatorDays = useMemo(() => {
    const minutesPerDay = sliderHours * 60;
    if (minutesPerDay <= 0 || avgMinutesPerTopic <= 0) return Infinity;
    const topicsPerDay = minutesPerDay / avgMinutesPerTopic;
    if (topicsPerDay <= 0) return Infinity;
    return Math.ceil(untouchedCount / topicsPerDay);
  }, [sliderHours, avgMinutesPerTopic, untouchedCount]);

  if (loading) {
    return <div style={{ padding: "40px", textAlign: "center", color: "var(--text-muted)" }}>Carregando seu painel...</div>;
  }
  if (error) {
    return <div style={{ padding: "40px" }}><p style={{ color: "var(--error)", marginBottom: "16px" }}>{error}</p><button onClick={() => void loadData()}>Tentar novamente</button></div>;
  }
    ...
    <div style={{ maxWidth: "1000px", margin: "0 auto", padding: "32px 20px" }}>
      ...
      <header style={{ marginBottom: "32px" }}>
        ...
        <p style={{ color: "var(--text)", fontSize: "16px" }}>
          {activeDiscipline === "redacao"
            ? "Redação é a disciplina ativa agora. Pratique uma redação para subir seu domínio."
            : `${activeDisciplineLabel} está esquecendo mais rápido — ${Math.max(1, Math.round(45 * (activeDays > 0 ? Math.min(2, 1 + activeDays / 10) : 1)))} min agora.`}
        </p>
      </header>
      ...
      <section style={{ marginBottom: "32px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
          ...
          <button style={{ background: "transparent", color: "var(--primary)", border: "none", fontSize: "14px", padding: "4px 8px" }} onClick={() => onNavigate("nucleo")}>
            Ver tudo →
          </button>
        </div>
            ...
            return (
              <div key={s.discipline} style={{ background: "var(--surface)", border: s.discipline === activeDiscipline ? "2px solid var(--primary)" : "1px solid var(--border)", borderRadius: "var(--radius)", padding: "16px" }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "4px" }}>
                  ...
                  <span style={{ fontSize: "12px", fontWeight: 600, color: tierColor }}>{Math.round(s.averageMastery)}%</span>
                </div>
                <div style={{ height: "4px", background: "var(--border)", borderRadius: "2px", overflow: "hidden" }}>
                  <div style={{ height: "100%", width: `${s.averageMastery}%`, background: tierColor, borderRadius: "2px" }} />
                </div>
              </div>
            );
          })}
      ...
      <section style={{ marginBottom: "32px" }}>
        ...
        {dueReviews.length === 0 ? (
          <p style={{ color: "var(--text-muted)", fontSize: "14px" }}>Nenhuma revisão vencida. Tudo em dia!</p>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
            {dueReviews.map((r) => {
              const label = DISCIPLINES[r.disciplineId as DisciplineId]?.label ?? r.disciplineId;
              return (
                <div key={r.topicId} style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", padding: "12px 16px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span style={{ fontSize: "14px" }}>{label}</span>
                  <span style={{ fontSize: "12px", color: r.days >= 14 ? "var(--error)" : "var(--warning)" }}>{r.days} dias atrás</span>
                </div>
              );
            })}
          </div>
        ...
        <div style={{ background: "var(--surface)", borderRadius: "var(--radius)", padding: "24px", border: "1px solid var(--border)" }}>
          <p style={{ fontSize: "15px", marginBottom: "8px" }}>
            ...
            <strong style={{ color: "var(--primary)" }}>
              {isFinite(projectionDays) ? `${projectionDays} ${projectionDays === 1 ? "dia" : "dias"}` : "indeterminado"}
            </strong>
          </p>
          <p style={{ fontSize: "12px", color: "var(--text-muted)", marginBottom: "24px" }}>
            {untouchedCount} tópicos restantes · {newTopicsPerDay.toFixed(1)} tópicos novos/dia (média dos últimos 30 dias)
          </p>
          ...
          <div style={{ borderTop: "1px solid var(--border)", paddingTop: "20px" }}>
            ...
            <div style={{ marginBottom: "12px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "8px" }}>
                <span style={{ fontSize: "13px", color: "var(--text-muted)" }}>Horas por dia</span>
                <span style={{ fontSize: "13px", fontWeight: 600 }}>{sliderHours}h</span>
              </div>
              <input
                type="range" min={1} max={8} step={1} value={sliderHours}
                onChange={(e) => setSliderHours(Number(e.target.value))}
                style={{ width: "100%" }}
              />
            </div>
            <p style={{ fontSize: "15px" }}>
              Nesse ritmo, você cobriria todos os tópicos em{" "}
              <strong style={{ color: "var(--primary)" }}>
                {isFinite(calculatorDays) ? `${calculatorDays} ${calculatorDays === 1 ? "dia" : "dias"}` : "indeterminado"}
              </strong>
            </p>
            <p style={{ fontSize: "12px", color: "var(--text-muted)", marginTop: "4px" }}>
              Baseado em {avgMinutesPerTopic.toFixed(0)} min/tópico (média histórica real)
            </p>
          </div>
  ...
  return (
    <div
      style={{
        ...
        border: highlight ? "2px solid var(--error)" : "1px solid var(--border)",
