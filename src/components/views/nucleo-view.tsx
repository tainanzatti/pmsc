...
import {
  ...
  DISCIPLINES,
  type DisciplineId,
  type DisciplineSkipState,
  type TopicWithMastery,
  allocateTopicsForDiscipline,
  averageMasteryForDiscipline,
  consolidateTopics,
  daysSinceLastDisciplineReview,
  masteryToTier,
...
export default function NucleoView() {
  ...
  const activeDiscipline = useMemo(
    () => nextHeroDiscipline(topics, skipStates),
    [topics, skipStates],
  );

  const scores = useMemo(
    () => scoreDisciplines(topics, skipStates),
    [topics, skipStates],
  );

  const scoreMap = useMemo(() => {
    const m: Record<string, number> = {};
    for (const s of scores) m[s.discipline] = s.score;
    return m;
  }, [scores]);

  const handleSkip = useCallback(
    async (discipline: DisciplineId) => {
      setSkipping(true);
      try {
        ...
        const msg = err instanceof Error ? err.message : "Erro ao pular disciplina";
        setError(msg);
      } finally {
        setSkipping(false);
      }
  ...
  if (loading) {
    return (
      <div style={{ padding: "40px", textAlign: "center", color: "var(--text-muted)" }}>
        Carregando seu núcleo de estudos...
      </div>
    );
  }
    ...
    <div style={{ maxWidth: "1100px", margin: "0 auto", padding: "32px 20px" }}>
      <header style={{ marginBottom: "32px" }}>
        ...
        <p style={{ color: "var(--text-muted)", fontSize: "15px" }}>
          Soldado PMSC 2026 — banca Instituto AOCP. A disciplina em destaque é
          definida pelo seu desempenho real e tempo desde a última revisão.
        </p>
      </header>
      ...
      >
        {ALL_DISCIPLINE_IDS.map((id) => {
          const isActive = id === activeDiscipline;
          const isExpanded = id === expandedDiscipline;
          const isRedacao = id === "redacao";
          return (
            <DisciplineCard
              key={id}
              disciplineId={id}
              ...
              onToggle={() =>
                isActive
                  ? setExpandedDiscipline(isExpanded ? null : id)
                  : undefined
              }
      ...
      {skipConfirm && (
        <SkipConfirmModal
          discipline={skipConfirm}
          onCancel={() => setSkipConfirm(null)}
          onConfirm={() => void handleSkip(skipConfirm)}
          skipping={skipping}
        />
      )}
    </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        {!isActive && (
          <span style={{ fontSize: "20px", opacity: 0.5 }} title="Disciplina bloqueada">
            🔒
          </span>
        )}
        {isActive && (
      ...
      >
        ...
        {skipCount > 0 && (
          <span style={{ color: "var(--warning)" }}>
            pulada {skipCount}x (urgência ×{urgency.toFixed(1)})
          </span>
        )}
      </div>
      ...
      {isRedacao && !isExpanded && (
        ...
        >
          Tratamento especial — detalhes em breve.
        </p>
      )}
    </div>
  );
...
function ExpandedDiscipline({
  disciplineId,
  topics,
  skipStates,
  score,
  onSkip,
}: ExpandedDisciplineProps) {
    ...
    >
      ...
      >
        <p style={{ fontSize: "13px", color: "var(--text)" }}>
          <strong>Por que esta disciplina agora?</strong>
          <br />
          {reason}
        </p>
        <p
          ...
          {allocated.map(({ topic, reason: allocReason }) => (
            ...
            >
              <div>
                ...
                >
                  {allocReason === "primeiro-ciclo" ? "novo" : "reforço"}
                </span>
              </div>
              <span
                style={{
    ...
    >
      ...
      >
        ...
        <p style={{ fontSize: "14px", color: "var(--text-muted)", marginBottom: "24px" }}>
          ...
          registrado. O multiplicador de urgência aumenta e ela reaparece como
          ativa mais cedo.
        </p>
        <div style={{ display: "flex", gap: "12px", justifyContent: "flex-end" }}>
          ...
          <button
            ...
            disabled={skipping}
          >
            {skipping ? "Pulando..." : "Confirmar e pular"}
          </button>
        </div>
      </div>
    </div>
